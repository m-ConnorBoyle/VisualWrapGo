...
class LineContainer {
    ...
    this._lineContainer.on("pointerover", (e) => {
      if (!this._dragging) {
        ...
        this._lineContainer.children.forEach((child) => {
          if (
            localPosition.x > 0 &&
            localPosition.x < this._canvasWidth &&
            localPosition.y > 0 &&
            localPosition.y < this._canvasWidth
          ) {
            child.alpha = 0.5;
  ...
  onDragMove(event) {
    if (this._dragging) {
      const newPosition = this._lineContainer.draggingData.getLocalPosition(
        this._app.stage,
      );
      this._lineContainer.x +=
        newPosition.x - this._lineContainer.startPosition.x;
      this._lineContainer.y +=
        newPosition.y - this._lineContainer.startPosition.y;
  ...
  onPointerDown() {
    this._lineContainer.on("pointerdown", (event) => {
      ...
      this._lineContainer.startPosition = this._lineContainer.toGlobal(
        event.data.getLocalPosition(this._lineContainer),
      );
  ...
  endDrag(event) {
    ...
    this._app.view.removeEventListener(
      "pointermove",
      this.onDragMove.bind(this),
    );
